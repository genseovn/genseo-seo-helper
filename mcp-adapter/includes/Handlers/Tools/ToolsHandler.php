...
class ToolsHandler {
	...
	public function handle_tool_call( array $params, int $request_id = 0 ): array {
		...
		// Fix: empty PHP array [] fails JSON Schema 'object' validation.
		// Cast to stdClass so WP_Ability::validate_input() sees it as an object.
		if ( is_array( $args ) && empty( $args ) && ! empty( $ability_input_schema['type'] ) && 'object' === $ability_input_schema['type'] ) {
			$args = (object) $args;
		}
